...
serve(async (req) => {
  ...
  try {
    ...
    const systemPrompts: Record<string, string> = {
      ...
      reflection_prompt: `You are a reflective coaching guide for entrepreneurs. Based on the story the user just read, generate deep reflection prompts that help them connect the story to their own journey. Return JSON: { "prompts": [{ "question": string, "focus_area": string }], "key_takeaway": string, "action_step": string }`,

      generate_story: `You are an expert entrepreneurship storyteller. Create an inspiring, realistic startup story based on the given theme, domain, and stage. The story should feel authentic, include specific details, and offer actionable lessons. Return JSON: { "title": string, "content": string (400-600 words), "summary": string (1-2 sentences), "category": string, "tags": string[], "author_name": string (fictional but realistic), "author_role": string, "key_lessons": string[] }`,

      mood_recommendations: `You are an empathetic startup mentor. Based on the user's current mood and recent emotional patterns, recommend types of inspirational content that would be most helpful right now. Return JSON: { "recommended_categories": [{ "category": string, "reason": string, "emotional_benefit": string }], "encouraging_message": string, "self_care_tip": string }`,
    };
