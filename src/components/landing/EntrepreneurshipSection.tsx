  ...
  return (
    <section ref={ref} className="py-20 sm:py-28 md:py-36 bg-background-alt relative overflow-hidden">
      <div className="absolute top-8 sm:top-12 left-4 sm:left-6 md:left-12">
        <span className="font-display text-[80px] sm:text-[120px] md:text-[180px] text-grey-divider/30 leading-none select-none">05</span>
      </div>
      ...
      <div className="container mx-auto px-5 sm:px-6 relative z-10">
        <div className="max-w-5xl mx-auto">
          ...
          <motion.div
            ...
            className="grid grid-cols-3 gap-3 sm:gap-4 mb-12 sm:mb-16 max-w-md mx-auto"
          >
            {[
              { label: "Aptitude", sub: "What you can do" },
              { label: "Attitude", sub: "How you respond" },
              { label: "Articulation", sub: "How you express" },
            ].map((item, i) => (
              <motion.div
                key={item.label}
                ...
                className="relative rounded-2xl p-3 sm:p-5 text-center overflow-hidden bg-card border border-border shadow-soft"
              >
                <p className="font-display text-base sm:text-xl text-foreground">{item.label}</p>
                <p className="font-body text-[9px] sm:text-[10px] text-grey-meta mt-1">{item.sub}</p>
              </motion.div>
            ))}
          </motion.div>
            ...
            <div className="space-y-2 sm:space-y-3 order-1 lg:order-2">
              ...
              {signals.map((s, i) => (
                <motion.div
                  ...
                  className="flex items-start gap-3 sm:gap-4 bg-card rounded-xl sm:rounded-2xl p-3 sm:p-4 border border-border shadow-soft hover:shadow-card transition-shadow duration-300"
                >
                  <div className="w-9 sm:w-10 h-9 sm:h-10 rounded-lg sm:rounded-xl gradient-warm flex items-center justify-center shrink-0">
                    <s.icon size={16} className="text-primary-foreground sm:w-[18px] sm:h-[18px]" />
                  </div>
                  <div>
                    <h4 className="font-display text-sm sm:text-base text-foreground">{s.title}</h4>
                    <p className="font-body text-[10px] sm:text-xs text-muted-foreground leading-relaxed">{s.description}</p>
                  </div>
                </motion.div>
              ))}
              <motion.div
