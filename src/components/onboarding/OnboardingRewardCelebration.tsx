...
const OnboardingRewardCelebration = ({ emoji, title, description, onContinue }: OnboardingRewardCelebrationProps) => {
  ...
  useEffect(() => {
    const items = Array.from({ length: 30 }, (_, i) => ({
      ...
      color: ["hsl(45,80%,65%)", "hsl(230,40%,25%)", "hsl(158,17%,37%)", "hsl(10,60%,50%)", "hsl(280,60%,60%)"][Math.floor(Math.random() * 5)],
    }));
    setConfetti(items);
  }, []);
  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        >
          <div className="absolute inset-0 bg-gradient-to-b from-[hsl(45,80%,92%)] to-transparent opacity-50 pointer-events-none" />
          <div className="relative z-10 space-y-4">
            ...
            <Button
              onClick={onContinue}
              className="bg-[hsl(230,40%,25%)] text-[hsl(45,80%,65%)] rounded-full px-8 font-body font-semibold mt-2"
            >
