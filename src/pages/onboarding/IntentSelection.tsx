    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-6">
      ...
      >
        <div className="text-center space-y-2">
          <p className="font-body text-sm text-primary font-semibold uppercase tracking-wider">Step 2 of 4</p>
          ...
          {intents.map((intent, i) => (
            ...
            >
              <div className={`w-14 h-14 rounded-xl flex items-center justify-center mb-4 ${
                selected === intent.value ? "gradient-warm" : "bg-muted"
              }`}>
                <intent.icon size={28} className={selected === intent.value ? "text-primary-foreground" : "text-muted-foreground"} />
              </div>
              ...
              <ul className="mt-4 space-y-1">
                {intent.features.map((f) => (
                  <li key={f} className="font-body text-xs text-primary font-medium">• {f}</li>
                ))}
              </ul>
            </motion.button>
          ))}
