...
export default function CareerMap() {
  ...
  const [zoomScale, setZoomScale] = useState<number>(1.5); // start zoomed in on user's location
  const [panOffset, setPanOffset] = useState<{ x: number; y: number }>({ x: 0, y: 0 });
  const visualTransform = useRef({ scale: 1.5, x: 0, y: 0 });
  ...
  // Sync state changes from external buttons to the visual transform tracker
  useEffect(() => {
    visualTransform.current = { scale: zoomScale, x: panOffset.x, y: panOffset.y };
    if (mapCanvasRef.current) {
      mapCanvasRef.current.style.transform = `translate(${panOffset.x}px, ${panOffset.y}px) scale(${zoomScale})`;
    }
  }, [zoomScale, panOffset]);
  ...
  useEffect(() => {
    ...
    const handleWheel = (e: WheelEvent) => {
      ...
      // Fine-tuned zoom factor for trackpad/mouse
      const zoomFactor = e.deltaY > 0 ? 0.85 : 1.15;
      
      const prevZoom = visualTransform.current.scale;
      const newZoom = Math.min(Math.max(prevZoom * zoomFactor, 0.05), 8.0);
      ...
      visualTransform.current = { scale: newZoom, x: newX, y: newY };

      if (mapCanvasRef.current) {
        mapCanvasRef.current.style.transform = `translate(${newX}px, ${newY}px) scale(${newZoom})`;
      }
  ...
  // Centre the world canvas in the viewport on first render
  useEffect(() => {
    const el = mapContainerRef.current;
    if (!el) return;
    ...
    // Default to the user's current career location (World Center coordinates)
    const targetX = 3000;
    const targetY = 2000;
    const startZoom = 1.5; 
    ...
    visualTransform.current = { scale: startZoom, x: initial.x, y: initial.y };

    if (mapCanvasRef.current) {
      mapCanvasRef.current.style.transform = `translate(${initial.x}px, ${initial.y}px) scale(${startZoom})`;
    }
  ...
  const { allPins, visiblePins, isLoading: dataLoading, statusMessage: dataStatus, totalCount } = useCareerMapData({
  ...
  // ── Hybrid LOD Canvas Engine (Draw 18,000 dots fast) ────────────────────────
  useEffect(() => {
    ...
    // Group pins by color to minimize ctx state changes
    const pinsByColor: Record<string, typeof allPins> = {};
    for (const pin of allPins) {
      const color = getCategoryColor(pin.category).border; // Use darker border color for visibility
      if (!pinsByColor[color]) pinsByColor[color] = [];
      pinsByColor[color].push(pin);
    }

    // Batch draw all 18,000 pins
    for (const [color, pins] of Object.entries(pinsByColor)) {
      ctx.fillStyle = color;
      ctx.beginPath();
      for (const pin of pins) {
        ctx.moveTo(pin.worldX, pin.worldY);
        ctx.arc(pin.worldX, pin.worldY, 5, 0, Math.PI * 2);
      }
      ctx.fill();
    }
  }, [allPins]);

  // ── Data-Driven Map Cartography (Convex Hulls) ─────────────────────────────
  const sectorHulls = useMemo(() => {
    ...
    for (const pin of allPins) {
      ...
      grouped[pin.category].push({ x: pin.worldX, y: pin.worldY });
    }

    // Compute hull for each category
    const hulls: { category: string; polygon: string }[] = [];
    for (const [category, pts] of Object.entries(grouped)) {
      if (pts.length < 3) continue;
      
      const hull = getConvexHull(pts);
      const pointsStr = hull.map(p => `${p.x},${p.y}`).join(' ');
      hulls.push({ category, polygon: pointsStr });
    }
    return hulls;
  }, [allPins]);
  ...
  const handleOpenRole = (roleKey: string, dynamicPin?: any) => {
    ...
    if (role) {
      ...
      setZoomScale(0.85);
      setPanOffset({
        x: (viewportSize.w / 2) - role.pinCoordinates.x * 0.85,
        y: (viewportSize.h / 2) - role.pinCoordinates.y * 0.85
      });
    }
  };
  ...
  return (
    <div className="myraaha-app w-full space-y-4 px-1 sm:px-0 select-none">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-2">
        <div className="flex items-center gap-2.5 sm:gap-3">
          ...
          <div className="min-w-0">
            <h1 className="app-page-title text-primary truncate">CareerScape</h1>
      ...
      <div className="w-full rounded-2xl border border-border bg-slate-100 dark:bg-slate-950 overflow-hidden shadow-2xl relative">
        {/* Floating Map Status Overlay */}
        <div className="absolute bottom-6 left-4 z-30 bg-background/95 shadow-lg rounded-xl border border-border px-3 py-2 flex items-center gap-2 max-w-xs sm:max-w-md backdrop-blur-sm pointer-events-auto select-none transition-all duration-300">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[8.5px] text-muted-foreground leading-none mt-0.5">
              {dataStatus} • {visiblePins.length} visible in viewport
            </span>
          </div>
        </div>
        ...
        <div 
          ref={mapContainerRef}
          className="w-full relative bg-[#e8f0e4] dark:bg-[#111612] h-[calc(100vh-200px)] min-h-[600px] overflow-hidden select-none"
          style={{ cursor: isPanning ? "grabbing" : "grab" }}
          onMouseDown={(e) => {
            if (e.button === 0) {
              setIsPanning(true);
              setPanStart({ x: e.clientX - visualTransform.current.x, y: e.clientY - visualTransform.current.y });
            }
          }}
          onMouseMove={(e) => {
            if (isPanning) {
              ...
              visualTransform.current.x = newX;
              visualTransform.current.y = newY;
              
              if (mapCanvasRef.current) {
                mapCanvasRef.current.style.transform = `translate(${newX}px, ${newY}px) scale(${visualTransform.current.scale})`;
              }
        ...
        >
          ...
          >
            ...
            >
              <defs>
                {/* Generous blur filter to create the overlapping gradient borders */}
                <filter id="hull-blur" x="-50%" y="-50%" width="200%" height="200%">
                  <feGaussianBlur stdDeviation="150" />
                </filter>
              </defs>
              {sectorHulls.map((hull, i) => {
                const color = getCategoryColor(hull.category).bg;
                return (
                  <polygon 
                    key={i}
                    points={hull.polygon}
            ...
            {/* Hybrid LOD Canvas (18,000+ points drawn via GPU, 0 DOM overhead) */}
            <canvas 
            ...
            {/* Map Road Grid (Simulating Google Maps Grid) */}
            <div className="absolute top-[160px] left-0 right-0 h-1.5 bg-white/70 rounded-full z-0 pointer-events-none" />
            <div className="absolute top-[300px] left-0 right-0 h-1.5 bg-white/70 rounded-full z-0 pointer-events-none" />
            <div className="absolute top-0 bottom-0 left-[200px] w-1.5 bg-white/70 rounded-full z-0 pointer-events-none" />
            <div className="absolute top-0 bottom-0 left-[430px] w-1.5 bg-white/70 rounded-full z-0 pointer-events-none" />
            <div className="absolute top-[90px] left-0 right-0 h-[3px] bg-white/50 pointer-events-none" />
            <div className="absolute top-[230px] left-0 right-0 h-[3px] bg-white/50 pointer-events-none" />
            <div className="absolute top-[380px] left-0 right-0 h-[3px] bg-white/50 pointer-events-none" />
            <div className="absolute left-[100px] top-0 bottom-0 w-[3px] bg-white/50 pointer-events-none" />
            <div className="absolute left-[320px] top-0 bottom-0 w-[3px] bg-white/50 pointer-events-none" />
            <div className="absolute left-[550px] top-0 bottom-0 w-[3px] bg-white/50 pointer-events-none" />

            {/* Dynamic Real Role Pins (17,000+ dataset) */}
            {visiblePins.map((pin) => {
              const isSelected = selectedRole.id === pin.id;
              const isSaved = isRoleSaved(pin.id);
              const col = getCategoryColor(pin.category);

              // Level of detail check based on zoom
              const isLowZoom = zoomScale < 0.25;

              // Base circle styling
              const pinBg = col.border;
              const borderStyles = isSelected ? "border-[#FF3B30] border-4 scale-110 animate-pulse" : "border-white/90 border-2";
              ...
              if (isLowZoom) {
                // Render as simple aggregated color dots for clean global view
                return (
                  <div
                    key={`dynamic-${pin.id}`}
                    className="absolute cursor-pointer transition-transform duration-100 hover:scale-150 z-20"
                    style={{
                      top: `${pin.worldY}px`,
                      left: `${pin.worldX}px`,
                      transform: "translate(-50%, -50%)"
                    }}
                    onClick={() => handleOpenRole(pin.id, pin)}
                    title={pin.name}
                  >
                    <div 
                      className={`w-3.5 h-3.5 rounded-full shadow-sm hover:ring-2 hover:ring-white transition-all`} 
                      style={{ 
              ...
              return (
                <div
                  key={`dynamic-${pin.id}`}
                  className="absolute flex flex-col items-center cursor-pointer transition-all duration-150 hover:scale-110 z-20 group"
                  style={{
                    top: `${pin.worldY}px`,
                    left: `${pin.worldX}px`,
                    transform: "translate(-50%, -50%)"
                ...
                >
                  <div 
                    className={`w-8 h-8 rounded-full flex items-center justify-center shadow-md transition-transform ${borderStyles}`} 
                    style={{ backgroundColor: pinBg }}
                  >
                    {renderIcon(14)}
                  </div>
                  <div className="flex items-center gap-0.5 text-[8.5px] font-bold text-[#2c2c2a] bg-white/95 px-1.5 py-0.5 rounded shadow border border-black/10 mt-1 whitespace-nowrap">
                    {pin.name}
                    {isSaved && <Star size={8} className="text-amber-500 fill-amber-500 ml-0.5" />}
                  </div>
                  <div className="text-[6.5px] text-white bg-black/60 rounded px-1 mt-0.5 opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
                    {pin.sectorName || col.label}
                  </div>
                </div>
              );
            })}
            ...
            {[
              ...
              <div
                ...
                style={{ top: emerging.y, left: emerging.x }}
            ...
            {coExploreActive && (
              <div 
                ...
                style={{ top: `${coExploreCursor.y}px`, left: `${coExploreCursor.x}px` }}
              >
          ...
          {/* FLOATING ACTION BUTTON — spec L340–349, L425 */}
          {!navigationMode && (
            <div className="absolute bottom-20 left-4 z-40 flex flex-col gap-2 items-start transition-all duration-300">
              {/* FAB quick actions */}
              {[
                { icon: "📍", label: "Pin Role", action: () => { if (selectedRole) toggleSaveRole(selectedRole.id); } },
                { icon: "🎯", label: "Compare 2 Roles", action: () => { setSidebarMode("comparison"); setSelectedRoleOpen(true); } },
                { icon: "🗺️", label: "Plan My Path", action: () => { setSidebarMode("pathfinder"); setSelectedRoleOpen(true); } },
                { icon: "🔔", label: "Alert Me (hiring spike)", action: () => alert("You'll be notified when hiring spikes for " + selectedRole.name + " in your city!") }
              ].map((fab, i) => (
                <button
                  key={i}
                  onClick={fab.action}
                  className="ghost w-10 h-10 bg-background border border-border rounded-full shadow-lg flex items-center justify-center text-base hover:bg-muted transition-colors"
                  title={fab.label}
                >
                  {fab.icon}
                </button>
              ...
              {/* Primary FAB */}
              <button
                className="ghost w-12 h-12 bg-[#3B8BD4] hover:bg-[#185FA5] text-white rounded-full shadow-xl flex items-center justify-center text-xl transition-colors"
                title="Quick Actions"
                onClick={() => { setSidebarMode("menu"); setSelectedRoleOpen(true); }}
              >
                +
              </button>
            </div>
          ...
          {/* EXPLORE CHIP BOTTOM BAR — spec L425, L459 (persistent collapsed bottom tray) */}
          {!navigationMode && (
            <div className="absolute bottom-3 left-[400px] right-20 z-40 flex items-center gap-2 overflow-x-auto scrollbar-hidden pointer-events-auto">
              {[
                ...
                <button
                  key={i}
                  onClick={chip.action}
                  className="ghost shrink-0 bg-background/95 backdrop-blur border border-border rounded-full px-3 py-1.5 text-[11px] font-semibold text-foreground shadow hover:bg-muted transition-colors whitespace-nowrap"
                >
                  {chip.label}
                </button>
              ))}
          ...
          <div className="absolute bottom-8 right-4 z-40 flex flex-col gap-3 items-end pointer-events-auto">
            {/* Layers Button */}
            <div className="relative">
              <button 
                onClick={() => setLayersPanelOpen(!layersPanelOpen)}
                className="w-10 h-10 bg-background border border-border rounded-xl shadow-md flex items-center justify-center text-foreground hover:bg-muted transition-colors cursor-pointer"
                title="Map Layers"
              >
                <Layers size={17} />
              </button>
            ...
            <button 
              onClick={() => {
                ...
                alert("Map Recentered: View reset to default scale & coordinates.");
              }}
              className="w-10 h-10 bg-background border border-border rounded-xl shadow-md flex items-center justify-center text-foreground hover:bg-muted transition-colors cursor-pointer"
              title="Recenter Map"
            >
              <Compass size={17} className="text-blue-500" />
            </button>
            ...
            <div className="bg-background border border-border rounded-xl shadow-md flex flex-col overflow-hidden">
              <button 
                onClick={() => setZoomScale(prev => Math.min(prev + 0.25, 8.0))}
              ...
              <button 
                onClick={() => setZoomScale(prev => Math.max(prev - 0.25, 0.05))}
          ...
          {isOfflineMode && (
            <div className="absolute top-3 left-1/2 -translate-x-1/2 z-50 bg-[#FFF9E6] dark:bg-amber-950/90 border border-amber-300 dark:border-amber-900 rounded-2xl p-3 shadow-2xl flex items-center gap-3 w-80 md:w-96 pointer-events-auto text-xs animate-in slide-in-from-top-4 duration-200">
          ...
          {navigationMode && (
            <div className="absolute top-3 left-3 right-3 z-40 max-w-md bg-[#2d2d2d] p-3.5 rounded-2xl flex items-center gap-3 border border-neutral-800 shadow-2xl">
          ...
          {/* LEFT SIDEBAR PANEL (Google Maps Style) */}
          {!navigationMode && (
            <div className="absolute top-3 left-3 bottom-3 w-[calc(100%-1.5rem)] md:w-[400px] z-30 flex flex-col pointer-events-none">
              {/* Universal Search Bar */}
              <div className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-lg p-1 pointer-events-auto shrink-0 mb-2">
                <div className="h-10 flex items-center gap-2 px-3.5">
                  {/* Google Maps Hamburger Menu */}
                  <button 
                    onClick={() => {
                      setSidebarMode(sidebarMode === "menu" ? "detail" : "menu");
                      setSelectedRoleOpen(true);
                    }}
                    className="ghost p-1.5 hover:bg-muted rounded-full cursor-pointer text-muted-foreground flex flex-col justify-center items-center gap-0.5"
                    title="Menu"
                  >
                    <div className="w-3.5 h-0.5 bg-neutral-600 dark:bg-neutral-300 mb-0.5" />
                    <div className="w-3.5 h-0.5 bg-neutral-600 dark:bg-neutral-300 mb-0.5" />
                    <div className="w-3.5 h-0.5 bg-neutral-600 dark:bg-neutral-300" />
                  </button>
                ...
                {searchFocused && (
                  <div className="border-t border-border/40 mt-1 max-h-[220px] overflow-y-auto">
                    {[
                      { name: "Product Manager", type: "💼", key: "pm" },
                      { name: "Software Engineer", type: "💼", key: "swengineer" },
                      { name: "UX Designer", type: "💼", key: "designer" },
                      { name: "Medical Doctor", type: "💼", key: "doctor" },
                      { name: "IAS Officer", type: "💼", key: "ias" },
                      { name: "Chartered Accountant", type: "💼", key: "ca" },
                      { name: "Advocate / Lawyer", type: "💼", key: "lawyer" },
                      { name: "Startup Founder", type: "💼", key: "founder" },
                      { name: "Google", type: "🏢", key: "pm" },
                      { name: "Razorpay Fintech", type: "🏢", key: "pm" },
                      { name: "Zomato Operations", type: "🏢", key: "pm" },
                      { name: "Myraaha Labs", type: "🏢", key: "founder" },
                      { name: "React & System Design", type: "🎯", key: "swengineer" },
                      { name: "UPSC Preparatory", type: "🗺️", key: "ias" }
                    ]
              ...
              {profileMenuOpen && (
                <div className="w-full bg-background border border-border rounded-xl shadow-xl pointer-events-auto mb-2 overflow-hidden">
                  ...
                  <div className="p-3 space-y-2 text-xs">
                    ...
                    <div className="flex gap-2">
                      <Button
                        onClick={() => { setOfflineDownloadDialogOpen(true); setProfileMenuOpen(false); }}
                        size="sm"
                        variant="outline"
                        className="flex-1 h-7 text-[10px] rounded-lg gap-1 border-[#3B8BD4] text-[#185FA5]"
                      >
                        📥 Offline Map Data
                      </Button>
                      <Button
                        onClick={() => {
                          ...
                          toast.success(nextState ? "Simulated Offline Mode. Live data disabled." : "Back Online. Live hiring pulse restored!");
                          setProfileMenuOpen(false);
                        }}
                        size="sm"
                        variant="outline"
                        className={`flex-1 h-7 text-[10px] rounded-lg gap-1 ${isOfflineMode ? "bg-amber-100 border-amber-300 text-amber-800 hover:bg-amber-200" : ""}`}
                      >
                    ...
                    <div className="flex gap-2">
                      <Button
                        onClick={() => {
                          const nextState = !isMentorView;
                          setIsMentorView(nextState);
                          toast.success(nextState ? "Entered Mentor Mode! Write reviews on role cards." : "Returned to Student View.");
                          setProfileMenuOpen(false);
                        }}
                        size="sm"
                        variant="outline"
                        className={`w-full h-7 text-[10px] rounded-lg gap-1 ${isMentorView ? "bg-purple-100 border-purple-300 text-purple-800 hover:bg-purple-200" : ""}`}
                      >
                        {isMentorView ? "🎓 Student View" : "🎓 Mentor Mode"}
                      </Button>
                    </div>
                  </div>
                </div>
              ...
              {selectedRoleOpen && !searchFocused && (
                <div className="w-full flex-1 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-2xl overflow-hidden flex flex-col pointer-events-auto min-h-0">
                  
                  {/* MODE: MENU */}
                  {sidebarMode === "menu" && (
                    <div className="flex flex-col h-full bg-card">
                      <div className="p-4 border-b border-border bg-muted/30 flex justify-between items-center">
                        <span className="font-bold text-sm text-foreground">CareerScape Explorer Menu</span>
                      ...
                      <div className="flex-1 overflow-y-auto p-3 space-y-1">
                        {[
                          ...
                          { id: "settings", label: "CareerScape Settings", icon: <Info size={15} className="text-muted-foreground" /> }
                        ].map(item => (
                          <div 
                            key={item.id}
                            onClick={() => setSidebarMode(item.id as typeof sidebarMode)}
                            className={`flex items-center gap-3 p-3 rounded-lg cursor-pointer transition-colors text-xs font-semibold ${
                              sidebarMode === item.id 
                      ...
                      <div className="p-3 border-t border-border bg-muted/10 text-center space-y-2">
                        ...
                        <span className="text-[10px] text-muted-foreground block">MyRaaha CareerScape v2.1</span>
                      </div>
                    </div>
                  )}
                  ...
                  {sidebarMode === "search_results" && (
                    <div className="flex flex-col h-full bg-card">
                      <div className="p-3.5 border-b border-border flex justify-between items-center shrink-0">
                        ...
                        <button onClick={() => { setSidebarMode("menu"); }} className="ghost text-muted-foreground hover:text-foreground cursor-pointer">
                          <X size={14} />
                        </button>
                      </div>
                            ...
                            >
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex justify-between items-start gap-1">
                                  ...
                                  <span className="text-[10px] text-emerald-600 font-bold bg-emerald-50 dark:bg-emerald-950/20 px-1.5 py-0.5 rounded shrink-0">{role.matchScore}% Match</span>
                                </div>
                                ...
                                <div className="flex justify-between items-center mt-2 pt-1 border-t border-border/40 text-[9.5px]">
                                  <span className="font-semibold text-foreground">{role.avgSalary}</span>
                        ...
                        {Object.values(ROLE_DETAILS_MOCK).filter(role => {
                          ...
                          <div className="text-center py-8 text-xs text-muted-foreground">
                            {isOfflineMode 
                              ...
                              : `No careers found matching "${searchQuery}". Try "Product" or "Engineer".`}
                          </div>
                        )}
                      </div>
                    </div>
                          ...
                          .map((role) => (
                            ...
                            >
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex justify-between items-start gap-1">
                                  <h4 className="font-bold text-xs text-foreground truncate">{role.name}</h4>
                                  <span className="text-[10px] text-[#3B8BD4] font-bold bg-[#E6F1FB] px-1.5 py-0.5 rounded shrink-0">{role.matchScore}% Match</span>
                                </div>
                                <p className="text-[10px] text-muted-foreground truncate mt-0.5">{role.category}</p>
                                <div className="flex justify-between items-center mt-2 pt-1 border-t border-border/40 text-[9.5px]">
                                  <span className="font-semibold text-foreground">{role.avgSalary}</span>
                              ...
                              <div className="border border-pink-200 dark:border-pink-900/50 bg-pink-50/10 rounded-2xl p-3.5 space-y-2 shadow-sm">
                                <div className="flex justify-between items-start">
                                  <div>
                                    ...
                                    <span className="text-muted-foreground text-[10px] block mt-0.5">94% Match · ₹12-20 LPA</span>
                                  </div>
                                  <Badge className="bg-pink-100 text-pink-700 border-none font-bold text-[8.5px]">94% Match</Badge>
                                </div>
                                ...
                                <div className="flex gap-2 pt-1.5">
                                  <Button 
                                    ...
                                    className="flex-1 bg-pink-500 hover:bg-pink-600 text-white font-bold h-7 rounded-lg text-[10px] border-none cursor-pointer"
                                  >
                                    Explore Node
                                  </Button>
                                  <Button 
                                    ...
                                    variant="outline"
                                    className="h-7 w-7 p-0 flex items-center justify-center rounded-lg border-pink-200 hover:bg-pink-50/40 text-pink-600 cursor-pointer"
                                  >
                                    <Bookmark size={11} />
                                  </Button>
                                </div>
                              ...
                              <div className="border border-border rounded-2xl p-3.5 bg-card space-y-2 shadow-sm">
                                <div className="flex justify-between items-start">
                                  <div>
                                    ...
                                    <span className="text-[10px] text-emerald-600 font-bold block mt-0.5">Demand ↗️ 45% this month</span>
                                  </div>
                                ...
                                <Button 
                                  ...
                                  variant="outline" 
                                  className="w-full h-7 text-[10px] font-bold rounded-lg border-border hover:bg-muted/40 cursor-pointer"
                                >
                                  Learn More
                                </Button>
                              </div>
                              ...
                              <div className="border border-border rounded-2xl p-3 bg-card divide-y divide-border/60">
                                {[
                                  ...
                                  >
                                    ...
                                    <span className="text-pink-600 font-bold text-[9.5px]">{gem.match}</span>
                                  </div>
                                ))}
                              </div>
                            </div>
                              ...
                              <div className="border border-border rounded-2xl p-3 bg-card space-y-2 text-[10.5px]">
                                ...
                                <div className="space-y-1.5">
                                  <div className="flex justify-between items-center bg-muted/20 p-2 rounded-xl">
                                    ...
                                    <Badge className="bg-[#E6F1FB] text-[#185FA5] border-none font-bold text-[8.5px]">Close 10% gap</Badge>
                                  </div>
                                  <div className="flex justify-between items-center bg-muted/20 p-2 rounded-xl">
                                    <span className="font-semibold text-foreground">Public Speaking & Presenting</span>
                                    <Badge className="bg-[#E6F1FB] text-[#185FA5] border-none font-bold text-[8.5px]">Close 8% gap</Badge>
                                  </div>
                                  <div className="flex justify-between items-center bg-muted/20 p-2 rounded-xl">
                                    <span className="font-semibold text-foreground">Startup Financial Modeling</span>
                                    <Badge className="bg-[#E6F1FB] text-[#185FA5] border-none font-bold text-[8.5px]">Close 5% gap</Badge>
                                  </div>
                                </div>
                              </div>
                            </div>
                  ...
                  {sidebarMode === "settings" && (
                    <div className="flex flex-col h-full">
                      <div className="p-3.5 border-b border-border flex justify-between items-center shrink-0">
                        <span className="font-bold text-xs text-foreground flex items-center gap-1.5">
                          <Info size={14} className="text-muted-foreground" /> CareerScape Settings
                        </span>
                        <button onClick={() => setSelectedRoleOpen(false)} className="ghost text-muted-foreground hover:text-foreground cursor-pointer">
                          <X size={14} />
                        </button>
                  ...
                  {sidebarMode === "realitycheck" && (
                    <div className="flex flex-col h-full">
                      ...
                      <div className="flex-1 overflow-y-auto p-4 space-y-3 scrollbar-hidden">
                        <p className="text-muted-foreground text-[11px] leading-relaxed">
                          Have workspace experience? Anonymously submit ratings & day-in-life reflections to refine CareerScape GPS coordinates.
                        </p>
                      ...
                      <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-hidden text-xs">
                        ...
                        <div className="border border-border rounded-2xl p-3.5 bg-card shadow-sm space-y-2">
                          ...
                          <div className="space-y-1.5 text-[10.5px] text-muted-foreground">
                            <div>✓ Early access to new CareerScape tools</div>
                    ...
                    <div className="flex flex-col h-full bg-background">
                      ...
                      <div className="w-full h-24 bg-slate-800 relative overflow-hidden shrink-0">
                        ...
                        <div className="absolute bottom-2 left-4 right-4 text-white">
                          <h2 className="text-sm font-bold leading-tight text-white">{selectedRole.name}</h2>
                          <span className="text-[10px] text-neutral-300 block">{selectedRole.category}</span>
                        </div>
                        <button 
                          onClick={() => setSelectedRoleOpen(false)}
                          className="ghost absolute top-2 right-2 w-6 h-6 rounded-full bg-black/40 hover:bg-black/60 flex items-center justify-center text-white cursor-pointer"
                        >
                          <X size={12} />
                        </button>
                      </div>
                      ...
                      <div className="p-3 border-b border-border shrink-0">
                        <div className="flex justify-between items-start">
                          <div>
                            <div className="flex items-center gap-1.5 flex-wrap">
                              <span className="font-bold text-xs text-foreground">{selectedRole.avgSalary} average</span>
                              <Badge className="bg-[#E6F1FB] text-[#185FA5] hover:bg-[#E6F1FB] text-[9.5px] border-none font-bold py-0 px-1.5">
                        ...
                        <div className="flex items-center gap-1.5 mt-2 overflow-x-auto scrollbar-hidden pb-0.5">
                          <Button 
                            ...
                            size="sm"
                            className="bg-[#3B8BD4] hover:bg-[#185FA5] text-white text-[10px] h-7 px-3 rounded-full flex items-center gap-1 shrink-0 font-semibold"
                          >
                            <Navigation size={11} className="rotate-45" /> Plan My Path
                          </Button>
                          <Button 
                            onClick={() => setDreamBoardQuickSaveOpen(true)}
                            size="sm"
                            variant="outline"
                            className="text-[10px] h-7 px-3 rounded-full flex items-center gap-1 shrink-0 font-semibold"
                          >
                            <Bookmark size={11} className={isRoleSaved(selectedRole.id) ? "fill-[#3B8BD4] text-[#3B8BD4]" : ""} />
                          ...
                          <Button 
                            ...
                            size="sm"
                            variant="outline"
                            className="text-[10px] h-7 px-3 rounded-full flex items-center gap-1 shrink-0 border-pink-500 text-pink-600 hover:bg-pink-50/20 font-semibold"
                          >
                            📸 Skill Gap AR
                          </Button>
                        </div>
                      ...
                      <div className="flex border-b border-border overflow-x-auto scrollbar-hidden bg-muted/20 shrink-0">
                        {[
                          ...
                          { id: "Skills", label: "Skills Required", icon: "🧠" },
                          { id: "MatchScore", label: "Match Score", icon: "🎯" },
                          { id: "Salary", label: "Salary Progression", icon: "💰" },
                          { id: "Ladder", label: "Career Ladder", icon: "📈" },
                          { id: "ExamGates", label: "Exam Gates", icon: "🚦" },
                          { id: "Education", label: "Education Paths", icon: "🎓" },
                          { id: "Companies", label: "Companies Hiring", icon: "🏢" },
                          { id: "Cities", label: "City Opportunities", icon: "🌆" },
                          { id: "AIImpact", label: "AI Impact", icon: "🤖" },
                          { id: "Similar", label: "Similar Roles", icon: "🔄" },
                          { id: "Lifestyle", label: "Lifestyle Reality", icon: "⚖️" },
                          { id: "RealityChecks", label: "Reality Checks™", icon: "💬" },
                          { id: "Workspace", label: "Workspace Tour", icon: "💻" },
                          { id: "Challenge", label: "Challenge Sim", icon: "⚡" },
                          ...
                          <button
                            ...
                            className={`ghost flex items-center gap-1 py-1.5 px-2.5 text-[11px] font-bold whitespace-nowrap border-b-2 transition-colors outline-none cursor-pointer ${
                        ...
                        {activeRoleTab === "ExamGates" && (
                          <div className="space-y-3 text-xs">
                            ...
                            {selectedRole.id === "doctor" && (
                              <div className="border border-[#791F1F] bg-[#FFF2F2] p-3 rounded-xl flex items-start gap-2.5">
                                <span className="w-5 h-5 bg-[#E24B4A] rounded text-white font-bold flex items-center justify-center text-[10px]">G</span>
                                <div>
                                  <span className="font-bold text-red-950 block">NEET Exam Gate Active</span>
                                  <p className="text-[10.5px] text-red-900 leading-snug mt-0.5">Required for all clinical medical roles in India. Minimum score: 580/720.</p>
                                </div>
                              </div>
                            )}
                            {selectedRole.id === "ias" && (
                              <div className="border border-[#791F1F] bg-[#FFF2F2] p-3 rounded-xl flex items-start gap-2.5">
                                <span className="w-5 h-5 bg-[#E24B4A] rounded text-white font-bold flex items-center justify-center text-[10px]">G</span>
                                <div>
                                  <span className="font-bold text-red-950 block">UPSC Civil Services Gate</span>
                                  <p className="text-[10.5px] text-red-900 leading-snug mt-0.5">3-stage exam process: Prelims, Mains, and interview. Under 0.1% selection rate.</p>
                                </div>
                              </div>
                            )}
                            {selectedRole.id !== "doctor" && selectedRole.id !== "ias" && (
                          ...
                          <div className="space-y-3">
                            ...
                            <div className="grid grid-cols-1 gap-2">
                              <div className="p-3 border border-border bg-card rounded-xl">
                                <span className="font-bold text-foreground block">Track A: Bootcamp Route</span>
                                <span className="text-[10px] text-muted-foreground block mt-0.5">PM bootcamp (UpGrad, Product School) + portfolio. Duration: 6-12 months. Cost: ₹80K.</span>
                              </div>
                              <div className="p-3 border border-border bg-card rounded-xl">
                                <span className="font-bold text-foreground block">Track B: MBA Route</span>
                                <span className="text-[10px] text-muted-foreground block mt-0.5">Post-grad MBA from Tier 1/2 college + Campus placement. Duration: 2 years. Cost: ₹12L+.</span>
                              </div>
                            </div>
                          </div>
                        )}
          ...
          {offlineDownloadDialogOpen && (
            <div className="absolute inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center pointer-events-auto p-4">
          ...
          {showMeasureDistanceDialog && measureDistancePoints.length >= 2 && (
            <div className="absolute inset-0 bg-black/55 backdrop-blur-sm z-50 flex items-center justify-center pointer-events-auto p-4">
              <div className="bg-background border border-border w-full max-w-sm rounded-3xl p-5 shadow-2xl space-y-4 text-xs animate-in zoom-in-95 duration-200">
                <div className="flex justify-between items-center border-b border-border pb-3">
                  <span className="font-bold text-sm text-foreground flex items-center gap-1.5">
          ...
          {voiceNavigatorOpen && (
            <div className="absolute bottom-6 left-4 z-55 w-80 bg-slate-900 border border-slate-700/60 rounded-3xl p-4 shadow-2xl space-y-3 pointer-events-auto text-xs text-white animate-in slide-in-from-bottom duration-250">
              {/* Header */}
          ...
          {dreamBoardCreateCollectionOpen && (
            <div className="absolute inset-0 bg-slate-950/60 backdrop-blur-xs z-50 flex items-center justify-center p-4 pointer-events-auto animate-in fade-in duration-150">
